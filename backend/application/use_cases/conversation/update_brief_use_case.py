...
class UpdateBriefUseCase:
    def __init__(self, conversation_repo: ConversationRepository):
        self._repo = conversation_repo

    async def execute(self, conversation_id: str, user_id: str, brief: dict) -> ConversationOutput:
        conversation = await self._repo.find_by_id(conversation_id, user_id)
        if not conversation:
            raise ConversationNotFoundError("Conversa não encontrada")

        conversation = await self._repo.update_brief(conversation_id, user_id, brief)
